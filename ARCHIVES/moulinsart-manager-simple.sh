...
launch_agents() {
    ...
    # Ajustement fin pour garantir 50/50
    tmux resize-pane -t "$SESSION_NAME:agents.0" -x 50%
    tmux resize-pane -t "$SESSION_NAME:agents.1" -x 50%
    tmux resize-pane -t "$SESSION_NAME:agents.0" -y 50%
    tmux resize-pane -t "$SESSION_NAME:agents.2" -y 50%
    ...
    # Lancer Claude dans chaque panel (ils sont déjà dans le bon répertoire)
...
resume_claude_in_agents() {
    echo -e "${YELLOW}🔄 Reprise de Claude avec --resume...${NC}"
    echo ""
    ...
    echo "• Arrêt de Claude dans chaque panel..."
    ...
    echo ""
    echo -e "${GREEN}✅ Claude relancé avec --resume!${NC}"
    echo -e "${CYAN}💡 Les agents ont gardé leur contexte de conversation${NC}"
    echo ""
