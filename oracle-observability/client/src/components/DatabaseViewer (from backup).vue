  ...
  <div class="database-viewer">
    ...
    <div v-if="viewMode === 'live'" class="live-feed">
      <TransitionGroup name="event-slide" tag="div">
        <div 
          v-for="event in liveEvents.slice(0, 20)" 
        ...
        >
          ...
          <div v-if="event.agent" class="event-agent">
            <span class="agent-badge">
              <AgentAvatar :agent="event.agent" size="small" />
              {{ event.agent }}
            </span>
          </div>
          ...
          <div class="filter-group">
            ...
            <select v-model="filters.agent" @change="fetchEvents">
              ...
              <option v-for="agent in availableFilters.agents" :key="agent" :value="agent">
                <AgentAvatar :agent="agent" size="small" /> {{ agent }}
              </option>
            </select>
          </div>
            ...
            <tr v-for="event in sortedEvents" :key="event.id" @click="showEventDetails(event)">
              ...
              <td class="event-agent">
                <span v-if="event.agent" class="agent-badge-small">
                  <AgentAvatar :agent="event.agent" size="small" /> {{ event.agent }}
                </span>
              </td>
              <td class="event-type">
...
// Computed
const totalPages = computed(() => Math.ceil(dbEvents.value.length / itemsPerPage.value))
...
const fetchEvents = async () => {
  try {
    ...
    console.log('🔍 Received events:', data.events?.length, 'events')
    console.log('🔍 Event types:', data.events?.map(e => e.type))
...
// Watch for live events changes (debugging)
watch(() => props.liveEvents, (newEvents) => {
  console.log('🔴 DatabaseViewer received new live events:', newEvents.length)
...
/* Removed transparent overlay to show full Moulinsart castle beauty */
...
.event-type {
  font-weight: 600;
  color: #2c3e50;
  font-size: calc(0.8em * var(--scale-factor, 1));
}

.event-time {
  color: #7f8c8d;
  font-size: calc(0.7em * var(--scale-factor, 1));
}
...
.event-content {
  ...
  line-height: calc(1.3 * var(--scale-factor, 1));
}

.event-data {
  margin-bottom: calc(2px * var(--scale-factor, 1));
  padding: calc(4px * var(--scale-factor, 1));
  background: rgba(248, 249, 250, 0.4);
  border-radius: calc(3px * var(--scale-factor, 1));
